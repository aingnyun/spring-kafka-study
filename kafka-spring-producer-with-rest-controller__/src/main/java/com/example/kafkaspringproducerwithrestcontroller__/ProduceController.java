...
public class ProduceController {
    ...
    public void selectColor(
            ...
            @RequestParam(value = "user") String userName) throws JsonProcessingException {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZZ");
        Date now = new Date();

        UserEventVO userEventVO = new UserEventVO(dateFormat.format(now), colorName, userName);

        ObjectMapper objectMapper = new ObjectMapper();
        String jsonColorLog = objectMapper.writeValueAsString(userEventVO);
